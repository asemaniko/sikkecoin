...
#if defined(NDEBUG)
# error "Sikkecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x43a2796501d313ecaaf8941b32fb6ebd12ff315963e1f24544cc8bbc606a33fe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sikkecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sikkecoin Signed Message:\n";
...
{
    // Sikkecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Sikkecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sikkecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sikkecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sikkecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sikkecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x89e35349b3564957d294a2b33f6d52c2aa39e4ee8f146790d89337a6aa56d751");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "utku kacar deneme";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1515442718;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085342561;

        if (fTestNet)
        {
            block.nTime    = 1515442674;
            block.nNonce   = 385466833;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4a68a56f8ed08383a1fb968b68d76eb920bb49f7f1b0393c29224f016f8adbf6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb3, 0xd2 }; // Sikkecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SikkecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SikkecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SikkecoinMiner(CWallet *pwallet)
{
    printf("SikkecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sikkecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SikkecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SikkecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SikkecoinMiner, pwallet));
}
